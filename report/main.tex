...
We found that the relation between the rank and the nuclear norm is the same as the relation between the \(\ell_0\) norm and the \(\ell_1\) norm. The rank of a matrix is equal to the number of nonzero singular values, and the nuclear norm is the sum of the absolute value of singular values, while \(\ell_0\)-norm is equal to the number of non-zero components of the vector and \(\ell_1\)-norm is the sum of the absolute value of the components of the vector. From this, we found a convex relaxation which is essentially the same as the relaxation for \(ell_0\)-norm minimization. Then, we give the proof of Theorem\ref{thr:rlx}
\begin{proof}
    Because \(X^\ast\) is the optimal point of the nuclear norm minimization,
    \[\begin{aligned}
        \minimize & \sum_{i=1}^k\sigma_i(X)\\
        \subjectto & \mathcal A(X)=b.
    \end{aligned}\]
    Denote \((x_1,x_2,\dots ,x_k)\) as \(\bar{x}\) and let \(\sigma_i(X^*) = x_i\). Then \(\bar{x}\) is the optimal solution of a \(\ell_1\)-norm minimization problem,
    \[\begin{aligned}
        \minimize & \norm x_1\\
        \subjectto & \mathcal{B}x=b.
    \end{aligned}\]
    with the RNP condition \(\Null{\mathcal{B}}\cap C(S) = \{0\}\). Thus \(\bar{x}\) is also a optimal solution of a \(\ell_0\)-norm minimization problem,
    \[\begin{aligned}
        \minimize & \norm x_0\\
        \subjectto & \mathcal{B}x=b.
    \end{aligned}\]
    The \(\ell_0\) norm of \(\bar{x}_0\) is actually the rank of \(X^*\). Now, we suppose that the \(rank X_0 < rank X^*\)
\end{proof}
...
\[
\left(1-\delta_{r}(\mathcal{A})\right)\|X\|_{F} \leq\|\mathcal{A}(X)\| \leq\left(1+\delta_{r}(\mathcal{A})\right)\|X\|_{F}
\]
holds for all matrices \(X\) of rank at most \(r\).
\end{definition}

\begin{theorem}\label{thr:rip}
    Suppose that \(r \geq 1\) is that \(\delta_{5r} < 1/10\), Then \(X^* = X_0\), where \(X_0\) is the optimal solution of low rank minimization and \(X^*\) is the optimal soultion of nuclear norm minimization.
\end{theorem} 

 Actually, through some analysis in probability, we found that the requirements of Theorem \ref{thr:rip} is easy to get, which shows the strength of Theorem \ref{thr:rip}. However, the proof of this theorem is hard, readers can find detailed proofs and explanations in the paper \emph{Guaranteed minimum-rank solutions of linear matrix equations via nuclear norm minimization}\cite{recht2010guaranteed}
